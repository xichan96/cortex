package docker

import (
	"context"
	_ "embed"
	"fmt"

	"github.com/xichan96/cortex/agent/types"
	"github.com/xichan96/cortex/pkg/errors"
)

type RemoveContainerTool struct{ baseTool }

func (t *RemoveContainerTool) Name() string { return "docker_remove_container" }

//go:embed docker_remove_container.txt
var dockerRemoveContainerDescription string

func (t *RemoveContainerTool) Description() string {
	return dockerRemoveContainerDescription
}
func (t *RemoveContainerTool) Schema() map[string]interface{} {
	return map[string]interface{}{
		"type": "object",
		"properties": map[string]interface{}{
			"container_id": map[string]interface{}{"type": "string", "description": "Container ID or name"},
		},
		"required": []string{"container_id"},
	}
}
func (t *RemoveContainerTool) Execute(ctx context.Context, input map[string]interface{}) (interface{}, error) {
	id, ok := input["container_id"].(string)
	if !ok || id == "" {
		return nil, errors.EC_TOOL_PARAMETER_INVALID.Wrap(fmt.Errorf("container_id is required"))
	}
	if err := t.client.DeleteContainer(ctx, id); err != nil {
		return nil, errors.EC_TOOL_EXECUTION_FAILED.Wrap(err)
	}
	return map[string]interface{}{"message": "removed", "container_id": id}, nil
}
func (t *RemoveContainerTool) Metadata() types.ToolMetadata {
	return types.ToolMetadata{SourceNodeName: "docker_remove_container", IsFromToolkit: false, ToolType: "docker"}
}
