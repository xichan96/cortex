...
> 状态：**待排期**。每项标注工作量、收益、依赖、是否需产品决策。

---

## P1 · 低成本高收益（建议下一轮先做）

### P1.1 `ReasoningTokens` 回填（prompt caching O1）
...
### P2.2 `delegate_return_mode` 开关（子代理 §14）
...
### P2.3 `spawn_agent` vs 扩 `delegate_to_agent` schema（子代理 §14）
...
### P2.4 子代理权限 restrict-only（子代理评估）
...
### P3.1 Compaction 前缀保留（prompt caching Step 4）
...
### P3.3 子代理 S3/S4（mailbox + spawn + notifier + turn 唤醒）
...
### P3.4 `DeterministicCompact` → LLM 摘要替换（长期记忆/压缩）
